/*
 * pin_names.ino
 * Display the I/O pin numbers and names and various macro definitions of ESP32-C3 mini board with onboard 0.42" OLED display
 * or the generic ESP32-C3 super mini board
 * 
 * This is a pioarduino IDE project that will NOT compile in the Arduino IDE because of the missing board definition 
...
void iopins(void) {
  ...
  Serial.println(" GPIO 5 is attached to channel 1 of ADC 2, but the latter is not reliable and the pin must not be used as an anlogue input");
  ...
  #ifdef ARDUINO_ESP32C3_OLED_MINI
  Serial.println("  ARDUINO_ESP32C3_OLED_MINI");
  #else
  Serial.println("  ARDUINO_ESP32C3_OLED_MINI not defined");
  #endif 

  #ifdef ARDUINO_MINI_ESP32C3_OLED
  Serial.println("  ARDUINO_MINI_ESP32C3_OLED");
  #else
  Serial.println("  ARDUINO_MINI_ESP32C3_OLED not defined");
  #endif 
...
void setup() {
  ...
  //Serial.printf("BOOT_PIN in INPUT_PULLUP mode, bootbtn = %s\n", (bootbtn) ? "HIGH" : "LOW");
  iopins();
}
